 ...
 * supplied from a presiously run DpRt, or some defaults are used.
 * If the output filename is specified a FITS image is written with each object mask having it's object number
 * written in.
 * <pre>
 * object_test [-help][-median <value>][-background_sd <value>][-l[og_level] <level>] <filename> 
...
/**
 * Data created from object pixel list showing extent of each object.
 */
static unsigned short *Object_Mask_Data = NULL;
/**
 ...
 */
static float Median = 10.0;
/**
 * Standard deviation of background.
 */
static float Background_SD = 3.0;
/**
 * Log Level.
 */
...
int main(int argc, char *argv[])
{
	...
	if(!Load())
		return 3;
	/* calculate threshold */
	thresh = Median+10.0*Background_SD;
	fprintf(stdout,"thresh value = %f.\n",thresh);
	/* object detect */
	clock_gettime(CLOCK_REALTIME,&start_time);
	retval = Object_List_Get(Image_Data,Median,Naxis1,Naxis2,thresh,8,&object_list,
				 &seeing_flag,&seeing);
